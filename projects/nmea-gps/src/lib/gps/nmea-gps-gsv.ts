...
export class NmeaGpsGsv extends NmeaGps {
    ...
    validateFieldNumWithSignalId(fieldNum: number): boolean {
        const repeatedFieldNum = fieldNum - NmeaGpsGsv.FIELD_BASE_NUM;
        // console.log(`${fieldNum},${NmeaGpsGsv.FIELD_BASE_NUM},${repeatedFieldNum}`);
        if (repeatedFieldNum < 0) {
            return false;
        }

        if (0 !== repeatedFieldNum % NmeaGpsGsv.FIELD_SAT_NUM) {
            return false;
        }
    ...
    validateFieldNumWithoutSignalId(fieldNum: number): boolean {
        const repeatedFieldNum = fieldNum - NmeaGpsGsv.FIELD_BASE_NUM + 1;
        // console.log(`${fieldNum},${NmeaGpsGsv.FIELD_BASE_NUM},${repeatedFieldNum}`);
        if (repeatedFieldNum < 0) {
            return false;
        }
    ...
    parse(tid: TalkerId, mid: MessageId, message: string): Message {
        ...
        const repeatedFieldNum = splitted.length - NmeaGpsGsv.FIELD_BASE_NUM;
        const repeatedBlockNum = repeatedFieldNum / NmeaGpsGsv.FIELD_SAT_NUM;
        const offset = NmeaGpsGsv.FIELD_SAT_OFFSET;
        const svList: MessageGsvSatellite[] = [];
        for (let i = 0; i < repeatedBlockNum; i++) {
            const sat = {
                svid: Number.parseFloat(splitted[offset + i * NmeaGpsGsv.FIELD_SAT_NUM] + 0),
                elv: Number.parseFloat(splitted[offset + i * NmeaGpsGsv.FIELD_SAT_NUM] + 1),
                az: Number.parseFloat(splitted[offset + i * NmeaGpsGsv.FIELD_SAT_NUM] + 2),
                cno: Number.parseFloat(splitted[offset + i * NmeaGpsGsv.FIELD_SAT_NUM] + 3)
