...
export class NmeaStdGsa extends NmeaStd {
    ...
    valicateFieldNum(fieldNum: number): boolean {
        const repeatedFieldNum = fieldNum - NmeaStdGsa.FIELD_BASE_NUM;
        if (repeatedFieldNum < 0) {
            return false;
        }

        // TIPS: NmeaStdGsa.FIELD_SAT_NUM === 1 だと無意味なのでコメントアウト
        // if (0 !== repeatedFieldNum % NmeaStdGsa.FIELD_SAT_NUM) {
        //     return false;
        // }
    ...
    parse(tid: TalkerId, mid: MessageId, message: string): Message {
        ...
        const repeatedFieldNum = splitted.length - NmeaStdGsa.FIELD_BASE_NUM;
        const repeatedBlockNum = repeatedFieldNum / NmeaStdGsa.FIELD_SAT_NUM;
        const offset = NmeaStdGsa.FIELD_SAT_OFFSET;
        const svList: MessageGsaSatellite[] = [];
        for (let i = 0; i < repeatedBlockNum; i++) {
            const sat = {
                svid: Number.parseFloat(splitted[offset + i * NmeaStdGsa.FIELD_SAT_NUM]),
